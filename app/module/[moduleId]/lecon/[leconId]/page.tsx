...
export default function LeconPage() {
  ...
  const [view, setView] = useState<'texte'|'flashcards'|'quiz'>('texte')
  ...
  if (loading) return <div style={{fontFamily:'Sora,sans-serif',display:'flex',alignItems:'center',justifyContent:'center',height:'100vh',color:'#9898B8'}}>Chargement...</div>
  ...
  return (
    <div style={{fontFamily:'Sora,sans-serif',minHeight:'100vh',background:'#F8F7F5'}}>
      ...
      <nav style={{background:'white',borderBottom:'1px solid rgba(0,0,0,.08)',padding:'14px 32px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:100}}>
        <Link href="/dashboard" style={{fontSize:18,fontWeight:700,textDecoration:'none',color:'#0F0F1A'}}>Veltis<span style={{color:'#3B3BF9'}}>.</span></Link>
        <div style={{fontSize:13,color:'#4A4A6A'}}>Module {moduleId} · Leçon {leconId}/{mod.lecons.length}</div>
      ...
      <div style={{maxWidth:720,margin:'0 auto',padding:'40px 24px'}}>
        ...
        <div style={{display:'flex',gap:8,marginBottom:24}}>
          {(['texte','flashcards','quiz'] as const).map(tab => (
            <button key={tab} onClick={() => setView(tab)} style={{padding:'8px 18px',borderRadius:100,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'Sora,sans-serif',background:view===tab?'#3B3BF9':'white',color:view===tab?'white':'#4A4A6A',outline:view===tab?'none':'1px solid rgba(0,0,0,.08)'}}>
          ...
          <div>
            ...
            <div style={{display:'flex',justifyContent:'space-between',marginTop:16}}>
              <button
                ...
                style={{padding:'10px 20px',borderRadius:100,fontSize:12,fontWeight:600,border:'1px solid rgba(0,0,0,.08)',background:'white',cursor:cardIndex===0?'not-allowed':'pointer',opacity:cardIndex===0?.4:1,fontFamily:'Sora,sans-serif'}}
              >← Précédente</button>
          ...
          <div style={{background:'white',borderRadius:20,padding:28,border:'1px solid rgba(0,0,0,.08)'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
              {lecon.quiz[quizIndex].options.map((opt,i) => {
                let bg = 'white', border = '1px solid rgba(0,0,0,.1)', color = '#0F0F1A'
                if (answered !== null) {
                  if (i === lecon.quiz[quizIndex].reponse) { bg='#EAF3DE'; border='1px solid #3B6D11'; color='#27500A' }
                  else if (i === answered && answered !== lecon.quiz[quizIndex].reponse) { bg='#FCEBEB'; border='1px solid #A32D2D'; color='#791F1F' }
                }
                return (
                  <button key={i} onClick={() => handleAnswer(i)} style={{padding:'12px 14px',borderRadius:10,fontSize:13,textAlign:'left',cursor:answered!==null?'default':'pointer',background:bg,outline:border,color,fontFamily:'Sora,sans-serif',transition:'all .12s'}}>
                    {opt}
                  </button>
                )
              })}
            ...
            {answered !== null && (
              <div>
                ...
                <button onClick={nextQuestion} style={{background:'#3B3BF9',color:'white',padding:'12px 24px',borderRadius:100,fontSize:13,fontWeight:700,border:'none',cursor:'pointer',fontFamily:'Sora,sans-serif'}}>
                  {quizIndex < lecon.quiz.length-1 ? 'Question suivante →' : 'Voir les résultats'}
                </button>
              </div>
            )}
          </div>
        ...
        {view === 'quiz' && quizDone && (
          <div style={{background:'white',borderRadius:20,padding:40,textAlign:'center',border:'1px solid rgba(0,0,0,.08)'}}>
            ...
            <div style={{fontSize:14,color:'#4A4A6A',marginBottom:24,fontWeight:300}}>{score} / {lecon.quiz.length} bonnes réponses</div>
            <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
              {nextLecon ? (
                ...
                <Link href="/dashboard" style={{background:'#3B3BF9',color:'white',padding:'12px 24px',borderRadius:100,fontSize:13,fontWeight:700,textDecoration:'none'}}>
                  Module terminé 🎉
                </Link>
              )}
